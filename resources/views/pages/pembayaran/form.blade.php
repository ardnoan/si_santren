    ...
    <div class="card">
      <div class="card-body">
        <form action="{{ $route }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('jenis_pembayaran') is-invalid @enderror"
                ...
                <option value="spp" {{ old('jenis_pembayaran', $pembayaran->jenis_pembayaran ?? '') == 'spp' ? 'selected' : '' }}>SPP Bulanan</option>
                <option value="pendaftaran" {{ old('jenis_pembayaran', $pembayaran->jenis_pembayaran ?? '') == 'pendaftaran' ? 'selected' : '' }}>Biaya Pendaftaran</option>
            ...
            <div class="col-md-6 mb-3" id="bulanBayarDiv" style="{{ old('jenis_pembayaran', $pembayaran->jenis_pembayaran ?? '') == 'spp' ? '' : 'display:none;' }}">
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('status') is-invalid @enderror"
                name="status"
                required>
                <option value="lunas" {{ old('status', $pembayaran->status ?? 'lunas') == 'lunas' ? 'selected' : '' }}>Lunas</option>
          ...
          @php
          $showBukti = in_array(old('metode_pembayaran', $pembayaran->metode_pembayaran ?? ''), ['transfer', 'qris']);
          @endphp

          <div class="mb-3" id="buktiTransferDiv" style="{{ $showBukti ? '' : 'display:none;' }}">
          ...
          <div class="d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-success btn-lg">
              <i class="bi bi-check-circle"></i> {{ $isEdit ? 'Update Pembayaran' : 'Simpan Pembayaran' }}
            </button>
          </div>
        </form>
      </div>
